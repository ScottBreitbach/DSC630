...
```{r lm compare}
# From: https://cran.r-project.org/web/packages/performance/performance.pdf
# Load libraries
library(performance)  
...
by `GarageArea` and `MasVnrArea`, then `WoodDeckSF` and `ExterQual` to a lesser
extent. Let's look at a few of them.
...
# Ensemble Modeling
Using `SuperLearner()`  
Source: https://www.datacamp.com/community/tutorials/ensemble-r-machine-learning
...
it is weighted highly in the ensemble. Following this, `gbm` and `speedlm` are
...
cv.sl <- CV.SuperLearner(y, x, V=5, family = gaussian(),
                         SL.library = list("SL.speedlm", "SL.svm", "SL.gbm", 
                                           "SL.extraTrees"))
